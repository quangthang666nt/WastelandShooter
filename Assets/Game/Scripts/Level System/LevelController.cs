...
namespace Watermelon.LevelSystem
{
    public static class LevelController
    {
        ...
        private static void LoadRoom(int index)
        {
            ...
            {
                LevelEnvironment environmentData = activeWorldData.GetEnvironment(environments[i].EnvironmentType);
              
                if (environmentData == null)
                {
                    Debug.Log("[Level Controller] Not found environment with type: " + environments[i].EnvironmentType + " for the world: " + activeWorldData.name);
                    continue;
                }

                ActiveRoom.SpawnEnvironment(environmentData, environments[i]);
            }

            ObstacleEntityData[] obstacles = roomData.ObstacleEntities;
            for (int i = 0; i < obstacles.Length; i++)
            {
                LevelObstacle obstacleData = activeWorldData.GetObstacle(obstacles[i].ObstaclesType);

                if(obstacleData == null)
                {
                    Debug.Log("[Level Controller] Not found obstacle with type: " + obstacles[i].ObstaclesType + " for the world: " + activeWorldData.name);
                    continue;
                }

                ActiveRoom.SpawnObstacle(obstacleData, obstacles[i]);
            }
